...
function NatureImage({ src, label }: NatureImageType) {
  ...
  const { transform, filter } = useTransform(scrollYProgress, [0, 0.5, 1], {
    transform: ["scale(1.2)", "scale(1)", "scale(1.2)"],
    filter: [
  ...
  const scale = useTransform(scrollYProgress, [0, 0.3, 1], [2, 1, 1]);
  ...
  return (
    <div className="relative flex items-center justify-center overflow-clip">
      ...
      <motion.p
        className="zalando-sans-expanded absolute text-4xl text-white sm:text-5xl md:text-6xl lg:text-7xl"
  ...
  return (
    <main className="flex flex-col items-center pb-40">
      ...
      <button
        ...
        className="mt-8 -translate-y-2 cursor-pointer rounded-3xl bg-white px-5 py-2 text-lg drop-shadow-md/25"
      >
        Back to top
      </button>
    </main>
