...
export default function FallingSpheres() {
  ...
  const [configs, setConfigs] = useState<SphereConfig[]>(() =>
    Array.from({ length: numberOfSpheres }, createSphereConfig),
  );

  function handleSphereCountChange(newCount: number) {
    setNumberOfSpheres(newCount);
    setConfigs((prev) => {
      if (newCount > prev.length) {
        return [
          ...prev,
          ...Array.from({ length: newCount - prev.length }, createSphereConfig),
        ];
      }
  ...
  return (
    ...
    >
      <motion.div
        ...
        className={`fixed right-4 bottom-4 bg-white drop-shadow-md/25 ${optionsOpen ? "p-4" : "flex items-center justify-center"} `}
      >
        <AnimatePresence>
          {optionsOpen ? (
            <motion.div
            ...
            >
              ...
              <CustomSlider
                ...
                onChange={(_, value) =>
                  handleSphereCountChange(value as number)
                }
              />
              ...
              <button onClick={() => setOptionsOpen(false)}>
                <IoCloseOutline className="absolute right-2 bottom-1 size-12 text-neutral-700" />
              </button>
            </motion.div>
          ) : (
            <motion.button
            ...
            >
              <CiCircleInfo className="size-12 text-neutral-600" />
            </motion.button>
          )}
        </AnimatePresence>
      </motion.div>
